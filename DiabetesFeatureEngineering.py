#######################
# Yamac TAN - Data Science Bootcamp - Week 6 - Project 1
#######################
...
# BloodPressure: Kan Basıncı (Küçüktansiyon) (mm Hg)
...
df_ = pd.read_csv("Odevler/HAFTA_06 FEATURE ENGINEERING/PROJE_I/diabetes.csv")
df = df_.copy()
df.head(10)
df.isnull().sum()
df.shape
...
# Verisetinin genel resmi üzerinden ön inceleme yapıldığı zaman bazı değişkenlere ait değerlerde anormallikler
# gözlenmektedir. Örneğin BloodPressure, SkinThickness ve BMI değişkenlerinde 0 değerlerinin bulunması gibi.
...
for x in df.columns:
    for y in df.columns:
        if x != y:
            ...
            print("Correlation between " + x + " and " + y + " = " + str(correlation))
...
df.describe().T
# Glucose,BloodPressure,SkinThickness,Insulin ve BMI değerlerinde NaN değerlerin 0 ile ifade edildiği tahmin edilmiştir
...
df.loc[(df['BMI'] <= 30), "weight_status"] = "not_obesity"
